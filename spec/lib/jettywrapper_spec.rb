  ...
  describe Ldfwrapper do
    ...
    context "downloading" do
      context "with default file" do
        it "should download the zip file" do
          expect(Ldfwrapper).to receive(:open).with('https://github.com/boston-library/ldf-jetty/archive/master.zip').and_return(system ('true'))
          Ldfwrapper.download
        end
      end
      ...
      context "specifying the file" do
        it "should download the zip file" do
          expect(Ldfwrapper).to receive(:open).with('http://example.co/file.zip').and_return(system ('true'))
          Ldfwrapper.download('http://example.co/file.zip')
          expect(Ldfwrapper.url).to eq('http://example.co/file.zip')
        end
      end
      context "specifying the version" do
        it "should download the zip file" do
          expect(Ldfwrapper).to receive(:open).with('https://github.com/boston-library/ldf-jetty/archive/v9.9.9.zip').and_return(system ('true'))
          Ldfwrapper.hydra_jetty_version = 'v9.9.9'
          Ldfwrapper.download
        end
      end
    end
    ...
    context "unzip" do
      ...
      context "with default file" do
        it "should download the zip file" do
          ...
          expect(Ldfwrapper).to receive(:expanded_zip_dir).and_return('tmp/jetty_generator/interal_dir')
      ...
      context "specifying the file" do
        before do
          Ldfwrapper.url = 'http://example.co/file.zip'
        end
        it "should download the zip file" do
          expect(File).to receive(:exists?).and_return(true)
          expect(Ldfwrapper).to receive(:expanded_zip_dir).and_return('tmp/jetty_generator/interal_dir')
    ...
    describe "env" do
      ...
      it "should have a setter" do
        Ldfwrapper.env = "abc"
        expect(Ldfwrapper.env).to eq "abc"
      end

      it "should load the ENV['LDFWRAPPER_ENV']" do
        ENV['LDFWRAPPER_ENV'] = 'test'
        ENV['RAILS_ENV'] = 'test2'
        ENV['environment'] = 'test3'
        expect(Ldfwrapper.env).to eq "test"
      end
      ...
      it "should use the ENV['RAILS_ENV']" do
        ENV['RAILS_ENV'] = 'test2'
        ENV['environment'] = 'test3'
        expect(Ldfwrapper.env).to eq "test2"
      end

      it "should load the ENV['environment']" do
        ENV['environment'] = 'test3'
        expect(Ldfwrapper.env).to eq "test3"
      end

      it "should default to 'development'" do
        expect(Ldfwrapper.env).to eq "development"
      end
    ...
    context "config" do
      ...
      it "loads the application ldf_jetty.yml first" do
        expect(IO).to receive(:read).with('./config/ldf_jetty.yml').and_return("default:\n")
        config = Ldfwrapper.load_config
      end

      it "loads the application ldf_jetty.yml using erb parsing" do
        expect(IO).to receive(:read).with('./config/ldf_jetty.yml').and_return("default:\n  a: <%= 123 %>")
        config = Ldfwrapper.load_config
        config[:a] == 123
      end

      it "falls back on the distributed ldf_jetty.yml" do
        expect(File).to receive(:exists?).with('./config/ldf_jetty.yml').and_return(false)
      ...
      it "should take the env as an argument to load_config and the env should be sticky" do
        expect(IO).to receive(:read).with('./config/ldf_jetty.yml').and_return("default:\n  a: 1\nfoo:\n  a: 2")
        config = Ldfwrapper.load_config('foo')
        expect(config[:a]).to eq(2)
        expect(Ldfwrapper.env).to eq 'foo'
      end
    ...
    context "instantiation" do
      it "can be instantiated" do
        ...
        expect(ts.class).to eql(Ldfwrapper)
      end
      ...
      it "escapes the :solr_home parameter" do
        ts = Ldfwrapper.configure(@jetty_params.merge(:solr_home => '/path with spaces/to/solr'))
        command = ts.jetty_command
      ...
      it "has a pid if it has been started" do
        ...
        ts = Ldfwrapper.configure(jetty_params)
        allow_any_instance_of(Ldfwrapper).to receive(:process).and_return(double('proc', :start => nil, :pid=>5454))
        ts.stop
        ts.start
      ...
      it "can pass params to a start method" do
        ...
        ts = Ldfwrapper.configure(jetty_params)
        ts.stop
        allow_any_instance_of(Ldfwrapper).to receive(:process).and_return(double('proc', :start => nil, :pid=>2323))
        swp = Ldfwrapper.start(jetty_params)
      ...
      it "can get the status for a given jetty instance" do
        ...
        allow_any_instance_of(Ldfwrapper).to receive(:process).and_return(double('proc', :start => nil, :pid=>12345))

        jetty_params = {
          :jetty_home => File.expand_path("#{File.dirname(__FILE__)}/../../jetty")
        }
        Ldfwrapper.stop(jetty_params)
        expect(Ldfwrapper.is_jetty_running?(jetty_params)).to eql(false)
        Ldfwrapper.start(jetty_params)
        expect(Ldfwrapper.is_jetty_running?(jetty_params)).to eql(true)
        Ldfwrapper.stop(jetty_params)
      end

      it "can get the pid for a given jetty instance" do
        # Don't actually start jetty, just fake it
        allow_any_instance_of(Ldfwrapper).to receive(:process).and_return(double('proc', :start => nil, :pid=>54321))
        jetty_params = {
          :jetty_home => File.expand_path("#{File.dirname(__FILE__)}/../../jetty")
        }
        Ldfwrapper.stop(jetty_params)
        expect(Ldfwrapper.pid(jetty_params)).to eql(nil)
        Ldfwrapper.start(jetty_params)
        expect(Ldfwrapper.pid(jetty_params)).to eql(54321)
        Ldfwrapper.stop(jetty_params)
      end
      ...
      it "writes a pid to a file when it is started" do
        ...
        ts = Ldfwrapper.configure(jetty_params)
        allow_any_instance_of(Ldfwrapper).to receive(:process).and_return(double('proc', :start => nil, :pid=>2222))
        ts.stop
    ...
    context "wrapping a task" do
      it "wraps another method" do
        allow_any_instance_of(Ldfwrapper).to receive(:start).and_return(true)
        allow_any_instance_of(Ldfwrapper).to receive(:stop).and_return(true)
      ...
      it "captures any errors produced" do
        allow_any_instance_of(Ldfwrapper).to receive(:start).and_return(true)
        allow_any_instance_of(Ldfwrapper).to receive(:stop).and_return(true)
        expect(Ldfwrapper.instance.logger).to receive(:error).with("*** Error starting jetty: this is an expected error message")
        expect { error = Ldfwrapper.wrap(@jetty_params) do
    ...
    context "quiet mode", :quiet => true do
      it "inherits the current stderr/stdout in 'loud' mode" do
        ts = Ldfwrapper.configure(@jetty_params.merge(:quiet => false))
        process = ts.process
      ...
      it "redirect stderr/stdout to a log file in quiet mode" do
        ts = Ldfwrapper.configure(@jetty_params.merge(:quiet => true))
        process = ts.process
