...
class Ldfwrapper
  ...
  class << self
    ...
    def url
      @url ||= defined?(ZIP_URL) ? ZIP_URL : "https://github.com/boston-library/ldf-jetty/archive/#{hydra_jetty_version}.zip"
      @url
    end
    ...
    def load_config(config_name = env)
      @env = config_name
      jetty_file = "#{app_root}/config/ldf_jetty.yml"
      ...
      unless File.exists?(jetty_file)
        ...
        jetty_file = File.expand_path("../config/ldf_jetty.yml", File.dirname(__FILE__))
      end

      begin
        jetty_erb = ERB.new(IO.read(jetty_file)).result(binding)
      rescue
        raise("ldf_jetty.yml was found, but could not be parsed with ERB. \n#{$!.inspect}")
      end

      begin
        jetty_yml = YAML::load(jetty_erb)
      rescue
        raise("ldf_jetty.yml was found, but could not be parsed.\n")
      end

      if jetty_yml.nil? || !jetty_yml.is_a?(Hash)
        raise("ldf_jetty.yml was found, but was blank or malformed.\n")
      end
    ...
    def start(params)
      ...
      Ldfwrapper.configure(params)
      Ldfwrapper.instance.start
      return Ldfwrapper.instance
    end
  ...
  def check_java_version!
    if java_version
      @checked_java_version ||= Ldfwrapper.check_java_version!(java_command, java_version)
    end
  end
  ...
  def start
    ...
    if pid
      if Ldfwrapper.is_pid_running?(pid)
    ...
    if Ldfwrapper.is_port_in_use?(self.port)
  ...
  def startup_wait!
    ...
    Timeout::timeout(startup_wait) do
      sleep 1 until (Ldfwrapper.is_port_in_use? self.port)
    end
    rescue Timeout::Error
